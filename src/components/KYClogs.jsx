...
const KYClogs = () => {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    const response = await makeGetReq(
      `/v1/admin-logs?actionType=KYC&pageNo=${
        page + 1
      }&size=${pageSize}&adminID=${session.userId}`
    );
    ...
    const logsRows = response?.data?.map((log) => ({
      ...
      remarks:
        log?.action?.log?.Remark?.at(0).length > 0
          ? log?.action?.log?.Remark?.at(0)
          : "--",
    }));
    setLogs(logsRows);
  ...
  const fetchMobileLogs = useCallback(async () => {
    ...
    const response = await makeGetReq(
      ...
      }&size=${mobilePaginationModal.pageSize}&adminID=${session.userId}`
    );
    const logsRows = response?.data?.map((log) => ({
      id: log?.logID,
      createdOn: new Date(log.createdAt)?.toLocaleString(),
